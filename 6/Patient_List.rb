...
class Patient_List
  ...
  def alter_database
    @db_connection.delete_all("patients")
    @patients.each do |el|
  ...
  def read_from_database
    result = @db_connection.read_table("patients")
    list = Array.new()
    if result
      result.each do |el|
  ...
  def add(data)
    begin
      ...
      if @db_connection
        ...
        inst.id = @db_connection.get_id("patients")
      else
  ...
  def edit(surname=nil, name=nil, patronymic=nil, birthdate=nil)
    begin
      if surname
        ...
        if @db_connection
          @db_connection.edit_table(@current, "surname",
                                    self.find_by(0, @current).surname, "patients")
        end
      end
      if name
        self.find_by(0, @current).name = name
        if @db_connection
          @db_connection.edit_table(@current, "name",
                                    self.find_by(0, @current).name, "patients")
        end
      end
      if patronymic
        self.find_by(0, @current).patronymic = patronymic
        if @db_connection
          @db_connection.edit_table(@current, "patronymic",
                                    self.find_by(0, @current).patronymic, "patients")
        end
      end
      if birthdate
        self.find_by(0, @current).birthdate = birthdate
        if @db_connection
          @db_connection.edit_table(@current, "birthdate",
                                    Date.parse(self.find_by(0, @current).birthdate), "patients")
        end
      end
    rescue
    end
  ...
  def delete
    ...
    if @db_connection
      @db_connection.delete_from(@current,"patients")
    end
  end
